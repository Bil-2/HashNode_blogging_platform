...
const BlogEditor = ({ blogToEdit, onSave, onCancel }) => {
    ...
    const [category, setCategory] = useState(blogToEdit?.category || 'Technology');
    ...
    return (
        <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2 text-text-primary">
            ...
            <div>
                <label htmlFor="category" className="block text-sm font-medium text-white/70 mb-1">Category *</label>
                <select
                    id="category"
                    value={category}
                    onChange={(e) => setCategory(e.target.value)}
                    className="w-full bg-black/20 border border-white/20 rounded-lg p-2 text-white focus:border-indigo-500 focus:ring-indigo-500"
                    required
                >
                    {categories.map((cat) => (
            ...
            <div className="flex flex-wrap justify-end gap-3 pt-4 border-t border-glass mt-6">
                ...
                <button
                    ...
                    disabled={isUploading || (!title || !content || !category)}
                >
