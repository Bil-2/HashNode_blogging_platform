...
const UserProfilePage = () => {
    ...
    const fetchUserPosts = async (profileId) => {
        ...
        try {
            ...
            const endpoint = (loggedInUser?._id === profileId) ? '/posts/myposts' : `/posts/user/${profileId}`;
    ...
    const handleProfileUpdate = async (updatedData) => {
        ...
        try {
            ...
            const { data: updatedUser } = await axios.put('/users/profile', updatedData, config);
    ...
    const handleDelete = (blogId) => {
        ...
        setConfirmAction(() => async () => { 
            try {
                const token = sessionStorage.getItem('token');
                const config = { headers: { Authorization: `Bearer ${token}` } };
                await axios.delete(`/posts/${blogId}`, config);
